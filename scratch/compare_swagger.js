...
function getOldSwagger() {
  try {
    ...
    // If not found in HEAD, check if we can get it from another way
    throw error;
  }
}
...
function generateMarkdown(pathDiff, schemaDiff) {
...
try {
  ...
  const markdown = generateMarkdown(pathDiff, schemaDiff);
