...
import { useAPOverdue } from "@/hooks/use-ar-ap";
...
export default function APOverduePage() {
  ...
  const totalDebt = apData?.items?.reduce((sum, item) => sum + (item.overdueAmount || 0), 0) || 0;
  const totalVendors = apData?.total || 0;
          ...
          <Table className="min-w-full">
            <TableHeader className="sticky top-0 bg-muted/50 z-10">
              <TableRow>
                ...
                <TableHead className="text-right font-semibold">Số tiền quá hạn</TableHead>
                <TableHead className="text-center font-semibold">Số ngày quá hạn</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  ...
                  >
                    Không tìm thấy công nợ quá hạn nào.
                  </TableCell>
                </TableRow>
              ) : (
                apData.items.map((item, index) => {
                  const daysOverdue = item.overdueDays !== undefined ? item.overdueDays : 0;
                  return (
                    <TableRow key={item.stockInId || index}>
                      ...
                      <TableCell className="text-right font-semibold tabular-nums text-sm text-destructive">
                        {item.overdueAmount !== undefined ? formatCurrency(item.overdueAmount) : "—"}
                      </TableCell>
        ...
        {apData && apData.totalPages > 1 && (
          <div className="flex-shrink-0 flex items-center justify-between px-6 py-2 border-t bg-background">
            <p className="text-xs text-muted-foreground">
              Trang {currentPage} / {apData.totalPages} ({apData.total} nhà cung cấp)
            </p>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
