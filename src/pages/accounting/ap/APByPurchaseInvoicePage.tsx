...
import { useAPByPurchaseInvoice } from "@/hooks/use-ar-ap";
...
export default function APByPurchaseInvoicePage() {
  ...
  const totalAmount = apData?.items?.reduce((sum, item) => sum + (item.totalAmount || 0), 0) || 0;
  const totalPaid = apData?.items?.reduce((sum, item) => sum + (item.paidAmount || 0), 0) || 0;
  const totalDebt = apData?.items?.reduce((sum, item) => sum + (item.remainingAmount || 0), 0) || 0;
        ...
        <div className="flex-1 overflow-auto border-t">
          <Table className="min-w-full">
            <TableHeader className="sticky top-0 bg-muted/50 z-10">
              <TableRow>
                <TableHead className="w-[140px] font-semibold">Mã phiếu nhập</TableHead>
                <TableHead className="text-center font-semibold">Ngày nhập</TableHead>
                <TableHead className="text-center font-semibold">Hạn thanh toán</TableHead>
        ...
        {apData && apData.totalPages > 1 && (
          <div className="flex-shrink-0 flex items-center justify-between px-6 py-2 border-t bg-background">
            <p className="text-xs text-muted-foreground">
              Trang {currentPage} / {apData.totalPages} ({apData.total} hóa đơn)
            </p>
