...
import { useARSummaryByCustomerGroup } from "@/hooks/use-ar-ap";
import { formatCurrency, customerTypeLabels } from "@/lib/status-utils";
import { toast } from "sonner";

export default function ARSummaryByCustomerGroupPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Tổng hợp công nợ theo nhóm khách hàng</title>
        <meta
          name="description"
          content="Tổng hợp công nợ phải thu theo nhóm khách hàng"
        />
      </Helmet>
      <div className="h-auto flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 px-6 py-3 border-b bg-background">
          <div>
            <h1 className="text-xl font-bold tracking-tight">
              Tổng hợp công nợ theo nhóm khách hàng
            </h1>
            <p className="text-xs text-muted-foreground">
              Xem tổng hợp công nợ phải thu theo từng nhóm khách hàng
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto border-t">
          <Table className="min-w-full">
            <TableHeader className="sticky top-0 bg-muted/50 z-10">
              <TableRow>
                <TableHead className="font-semibold">Tên nhóm khách hàng</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  ...
                  >
                    Không tìm thấy dữ liệu công nợ theo nhóm khách hàng nào.
                  </TableCell>
                </TableRow>
              ) : (
                arData.items.map((item, index) => (
                  <TableRow key={index}>
                    <TableCell className="font-semibold text-sm">
                      {item.groupName
                        ? customerTypeLabels[item.groupName] || item.groupName
                        : "—"}
                    </TableCell>
        ...
        {arData && arData.totalPages > 1 && (
          <div className="flex-shrink-0 flex items-center justify-between px-6 py-2 border-t bg-background">
            <p className="text-xs text-muted-foreground">
              Trang {currentPage} / {arData.totalPages} ({arData.total} nhóm)
            </p>
