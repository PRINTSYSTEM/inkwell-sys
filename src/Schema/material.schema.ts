...
export const MaterialPropertiesSchema = z.object({
  ...
  colorCapability: z
    .enum(["full_color", "black_white", "spot_color"])
    .optional(),
  printMethods: z
    .array(z.enum(["digital", "offset", "screen", "flexo"]))
    .default([]),
...
export const MaterialSchema = z.object({
  ...
  code: z
    ...
    .regex(
      /^MAT-\d{4}-\d{6}$/,
      "Material code must follow format MAT-YYYY-XXXXXX"
    ),
...
export const MaterialFilterSchema = z.object({
  ...
  tags: z.array(z.string()).optional(),
  sortBy: z
    .enum(["name", "category", "createdAt", "lastUsed", "currentStock"])
    .default("name"),
