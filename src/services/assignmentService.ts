...
class AssignmentManagementService {
  ...
  static async getAssignmentSuggestions(
    assignmentId: string,
    ...
    const suggestions: AssignmentSuggestion[] = [
      {
        ...
        confidence: 0.92,
        reasons: [
          'Có kinh nghiệm 5 năm với loại công việc này',
          'Tỷ lệ hoàn thành đúng hạn 95%',
          'Hiện tại workload ở mức 70%'
        ],
        concerns: [],
      ...
      },
      {
        ...
        confidence: 0.88,
        reasons: [
          'Chuyên môn phù hợp 90%',
          'Có sẵn thời gian trong tuần tới'
        ],
      ...
      },
      {
        ...
        confidence: 0.75,
        reasons: [
          'Có thể học hỏi kỹ năng mới',
          'Workload hiện tại thấp'
        ],
        concerns: [
          'Cần thời gian làm quen với công việc',
          'Chưa có kinh nghiệm với loại dự án này'
        ],
  ...
  static async bulkAssignTasks(action: AssignmentBulkAction): Promise<Assignment[]> {
    ...
    const assignments = this.generateMockAssignments()
      .filter(a => action.assignmentIds.includes(a.id));
  ...
  static async addAssignmentComment(
    assignmentId: string,
    content: string,
    isInternal = false
...
export const assignmentService = AssignmentManagementService;