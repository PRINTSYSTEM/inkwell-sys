...
const mockAssignments: DesignAssignment[] = [
  {
    ...
    description: 'Thiết kế hộp đóng gói cho sản phẩm mới của khách hàng ABC Corp',
...
export class DesignAssignmentService {
  ...
  static async createAssignment(
    ...
    try {
      const validation = validateSchema(CreateDesignAssignmentSchema, assignmentData);
      if (!validation.success) {
        throw new Error(`Validation error: ${validation.errors.errors.map(e => e.message).join(', ')}`);
      }
  ...
  static async updateAssignmentStatus(
    ...
    try {
      const validation = validateSchema(UpdateDesignAssignmentSchema, updateData);
      if (!validation.success) {
        throw new Error(`Validation error: ${validation.errors.errors.map(e => e.message).join(', ')}`);
      }
  ...
  static async getDesignerWorkload(designerId: string): Promise<{
    ...
    try {
      ...
      const activeStatuses: DesignAssignmentStatus[] = ['pending', 'in_progress', 'review', 'revision'];
      const activeAssignments = designerAssignments.filter(
        assignment => activeStatuses.includes(assignment.status)
      );
   ...
   */
  static async getDepartmentStats(departmentId: string): Promise<DesignAssignmentStats> {
    try {
      await new Promise(resolve => setTimeout(resolve, 300));
      ...
      const overdue = assignments.filter(
        a => a.deadline < now && !['completed', 'cancelled'].includes(a.status)
      ).length;
      ...
      if (completedAssignments.length > 0) {
        const totalHours = completedAssignments.reduce((sum, assignment) => {
          const hours = (assignment.completedAt!.getTime() - assignment.startedAt!.getTime()) / 
                       (1000 * 60 * 60);
          return sum + hours;
        }, 0);
   ...
   */
  static async getAssignmentById(assignmentId: string): Promise<DesignAssignment | null> {
    try {
      await new Promise(resolve => setTimeout(resolve, 200));
