...
interface ProductionDelayReportModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function ProductionDelayReportModal({
  open,
  onOpenChange,
}: ProductionDelayReportModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] bg-white dark:bg-slate-900 border-slate-200 p-5 rounded-2xl flex flex-col gap-4 overflow-hidden">
        {/* Modal Header */}
        <DialogHeader className="pb-2 border-b flex flex-row items-center justify-between shrink-0">
          <DialogTitle className="flex items-center gap-2 text-slate-900 dark:text-slate-100 font-extrabold text-lg">
            <AlertCircle className="h-5 w-5 text-red-600" />
            Báo cáo Nhật ký LSX Trễ tiến độ
          </DialogTitle>
          <Button variant="outline" size="sm" className="h-8 text-xs font-semibold mr-6" onClick={handleRefresh}>
        ...
        <div className="flex-1 overflow-y-auto space-y-3.5 pr-1">
          ...
          {/* Main Table */}
          <Card className="shadow-xs flex-1 min-h-[300px]">
